...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // json 转模型
        let model = Student.codeble.deserialize(from: json)
        printLog(model)
        
        // 模型转 json
        let modelJSON = model?.codeble.toJSONString()
        printLog(modelJSON)
        
        // json 转字典
        let modelDict = modelJSON?.codeble.toDict()
        printLog(modelDict)
        
        // 模型转字典
        let modelDict1 = model?.codeble.toDict()
        printLog(modelDict1)
        
        // 字典转模型
        let model1 = Student.codeble.deserialize(from: modelDict!)
        printLog(model1)
        
        // json 转模型
        let model2 = Student.codeble.deserialize(from: modelJSON!)
        printLog(model2)
        // 字典转模型
        let model3 = Student.codeble.deserialize(from: modelDict1!)
        printLog(model3)
        ...
        printLog(array)
        // 数组转 json
        let arrayJSON = array.codeble.toJSONString()
        printLog(arrayJSON)
        // 数组转元素是字典的数组 [[String : Any]]
        let arrayDicts = array.codeble.toDicts()
        printLog(arrayDicts)
        
        // 根据 json 转模型的数组
        let array1 = [Student].codeble.deserialize(from: arrayJSON!)
        printLog(array1)
        ...
        let schoolJSON1 = Student.codeble.deserialize(from: dict1)!.codeble.toJSONString()!
        let schoolJSON2 = Student.codeble.deserialize(from: dict2)!.codeble.toJSONString()!
        let schoolJSON3 = Student.codeble.deserialize(from: dict3)!.codeble.toJSONString()!
        ...
        let array2 = [Student].codeble.deserialize(from: jsonArray)
        printLog(array2)
        
        // 根据元素是 json 的 NSArray 转模型数组
        let array3 = [Student].codeble.deserialize(from: jsonDictArray)
        printLog(array3)
        
        // 根据元素是字典的 array 转模型数组
        let array4 = [Student].codeble.deserialize(from: dictArray)
        printLog(array4)
        
        // 根据元素是字典的 NSArray 转模型数组
        let array5 = [Student].codeble.deserialize(from: dictNSArray)
        printLog(array5)
        
        // 自定义字段
        let personDict: [String : Any] = ["first_name" : "zhang", "age" : 10]
        let person = Person.codeble.deserialize(from: personDict)
